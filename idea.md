...
---
# On date : 21.08.26
The DHT22-vs-Wi-Fi conflict that was open as of the last update is now
resolved - not worked around, resolved. Root cause: DHT22's single-wire,
self-timed protocol has no tolerance for the Wi-Fi driver's background
task, and no amount of scheduling priority, power management, or
electrical shielding changes that, because the disruption isn't
electrical or scheduling-related, it's protocol-level. Switching to I2C
sensors - which have an actual clock line and are designed for exactly
this kind of shared-hardware environment - fixed it immediately and
completely. (It was in smart words of clode,when i wet to him, but simply - its me and my stupidity alongside - i just genualy did not check that this sensor DOESNT have a clock...and she claims to do a smart project for diplom...wtvr, so yeah - i spent s much time just bc i relay on my unattentive mind)

The system now runs two real sensors sharing one I2C bus properly - a
BME280 for temperature/humidity/pressure, and an MPU-6050 for the
vibration signal the FFT pipeline is built around - both reading
continuously and correctly with Wi-Fi fully active. The dashboard was
also rebuilt as a single consolidated view instead of several separate
pages, showing both sensors' live data together - real temperature,
humidity, pressure, and motion data, correctly classified, alongside a
live telemetry strip.

*(Add the dashboard screenshot here the same way as the other images in
this file - upload it to a GitHub issue/comment on the repo to get a
`user-attachments` URL, then drop in an `<img>` tag like the ones
above.)*

Full technical account of how this was diagnosed and fixed is in
`tech.md`, Sections 3.9-3.12.
